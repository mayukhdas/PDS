...
def evaluate_policy(env, policy, max_episode_steps, deterministic=True):
    ...
    for step in range(max_episode_steps):
        ...
        action = policy.act(torchify(obs), deterministic=deterministic).detach().cpu().numpy()
...
class QFGrad(nn.Module):
    ...
    def act(self, state, deterministic=True):
        ...
        return updated_action.squeeze(0)
...
meta_policy = QFGrad(iql.policy, iql.qf, beta=0.1)

# evaluate the policy
results = eval_policy(env, env_name, meta_policy, max_episode_steps=1000, n_eval_episodes=10)
...
plt.savefig(f"gravity_variation_{env_name}_qfgrad.png")
plt.show()
